          ...
          <TableBody>
            {Array.from({ length: 5 }).map((_, i) => (
              <TableRow key={i} className="border-b border-border/40 hover:bg-transparent">
                <TableCell className="px-4 py-3.5"><Skeleton className="h-5 w-3/4 rounded-md" /></TableCell>
                <TableCell className="px-4 py-3.5"><Skeleton className="h-5 w-1/2 rounded-md" /></TableCell>
                <TableCell className="px-4 py-3.5"><Skeleton className="h-5 w-2/3 rounded-md" /></TableCell>
                <TableCell className="px-4 py-3.5"><Skeleton className="h-5 w-4/5 rounded-md" /></TableCell>
                <TableCell className="px-4 py-3.5"><Skeleton className="h-8 w-8 rounded-full ml-auto" /></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
            ...
            >
              <TableCell className="px-4 py-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div 
                    className="font-semibold text-sm text-foreground max-w-[200px] xl:max-w-[250px] truncate cursor-pointer hover:underline"
              ...
              <TableCell className="px-4 py-3.5 text-sm text-muted-foreground">
                <div className="truncate max-w-[120px]" title={client.rut}>{client.rut || "—"}</div>
              </TableCell>
              <TableCell className="px-4 py-3.5 text-sm text-muted-foreground">
                <div className="truncate max-w-[170px]" title={client.contacto}>{client.contacto || "—"}</div>
              </TableCell>
              <TableCell className="px-4 py-3.5 text-sm text-muted-foreground">
                <div className="flex flex-col gap-0.5">
                  {client.email && <span className="truncate max-w-[220px]" title={client.email}>{client.email}</span>}
                  {client.telefono && <span className="truncate max-w-[220px]" title={client.telefono}>{client.telefono}</span>}
